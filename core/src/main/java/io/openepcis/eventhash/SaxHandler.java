...
public class SaxHandler extends DefaultHandler {
  ...
  public void startElement(final String uri, final String localName, final String qName, final Attributes attributes) {
    ...
    if (ConstantEventHashInfo.getContext().getFieldsToExcludeInPrehash().stream().noneMatch(getXMLPath()::contains)) {
      ...
      if (attributes.getLength() > 0) {
        ...
        for (int att = 0; att < attributes.getLength(); att++) {
          // If the attributes contain the : then consider them as namespaces otherwise as the fields such as type, source, etc. of the EPCIS event
  ...
  private void rootNodePopulater(final String qName) {
    // If EPCIS eventType is found then create a new rootNode to store and create fresh pre-hash string.
    ...
    } else if (currentNode != null && ConstantEventHashInfo.WHY_DIMENSION_XML_PATH.stream().anyMatch(getXMLPath()::startsWith)) {
      ContextNode n = new ContextNode(currentNode, null, (String) null);
      currentNode.children.add(n);
      currentNode = n;
    } else if (currentNode != null && ConstantEventHashInfo.WHAT_DIMENSION_XML_PATH.stream().noneMatch(getXMLPath()::startsWith)) {
  ...
  public void endElement(final String uri, final String localName, final String qName) {
    if (ConstantEventHashInfo.getContext().getFieldsToExcludeInPrehash().stream().noneMatch(getXMLPath()::contains)) {
      // Do not store the values for the fields which needs to be ignored such as EPCISDocument, EPCISBody, etc.
      if (rootNode != null && !ConstantEventHashInfo.EXCLUDE_XML_FIELDS.contains(qName)) {
        // Call the method to write the appropriate values. Splitting the method to avoid the cognitive complexity.
        xmlParser(qName);
      }

      // At the end of the every event in the document same the rootNode with the rootNodes and assign the rootNode with null for subsequent event.
      if (ConstantEventHashInfo.EPCIS_EVENT_TYPES.contains(qName)) {
        // After reading each XML event and converting it to ContextNode store the information in rootNodes.
        emitter.emit(rootNode);

        // After creating the pre-hash string and generating Hash-ID discard the rootNode information for subsequent event.
        rootNode = null;
      }
      ...
      path.pop();
    } else if (ConstantEventHashInfo.getContext().getFieldsToExcludeInPrehash().stream().anyMatch(getXMLPath()::contains)) {
      path.pop();
    }
  }

  // Private method called by endElement to write the values. Splitting the method to avoid the cognitive complexity.
  private void xmlParser(final String qName) {
    ...
    if (ConstantEventHashInfo.WHAT_DIMENSION_XML_PATH.stream().anyMatch(p::startsWith)) {
      ...
      if (ConstantEventHashInfo.WHY_DIMENSION_XML_PATH.stream().anyMatch(p::startsWith) || ConstantEventHashInfo.HOW_DIMENSION_XML_PATH.stream().anyMatch(p::startsWith)) {
        // If the attribute values are present within the XML then add them to attributes variable in context node as childrens
        ...
        // Add filtered attributes to children for userExtensions and other fields wherever attributes present
        if (MapUtils.isNotEmpty(currentAttributes)) {
