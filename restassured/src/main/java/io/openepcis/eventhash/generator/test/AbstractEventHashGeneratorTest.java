...
public abstract class AbstractEventHashGeneratorTest {
  ...
  public void invalidContentTypeTest() {
    Stream.of(documentApi(), eventsApi())
        .forEach(
            url -> {
              RestAssured.given()
  ...
  public void hashGeneratorDocumentComparisonTest() {
    List<URL> jsonCaptureFiles =
        ResourceFinder.searchResource("2.0", "json", "capture/documents", null);
    List<URL> xmlCaptureFiles =
        ResourceFinder.searchResource("2.0", "xml", "capture/documents", null);
    jsonCaptureFiles.stream()
        .forEach(
            url -> {
              try {
  ...
  public void hashGeneratorEventsComparisonTest() {
    List<URL> jsonCaptureFiles =
        ResourceFinder.searchResource("2.0", "json", "capture/events", null);
              ...
              try {
                ...
                if (xmlUrl != null) {
                  ...
                  if (!INVALID_HASH_EVENTS.stream().anyMatch(s -> xmlUrl.getFile().contains(s))) {
                    Assertions.assertEquals(
                        jsonResponse.body().asString(), xmlResponse.body().asString());
                  }
                }
              } catch (IOException e) {
                throw new RuntimeException(e);
